...
export const getCyprusCoinStatus = (year, denomination) => {
    const y = parseInt(year);
    const val = parseFloat(denomination);

    // Cyprus joined Euro in 2008
    ...
    // 5c
    if (val === 0.05) {
        if ([2008, 2013, 2015, 2016].includes(y)) return STATUS_CIRCULATION;
        return STATUS_SET_ONLY;
    }

    // 10c
    if (val === 0.10) {
        if ([2008, 2016, 2018, 2020, 2023, 2024, 2025].includes(y)) return STATUS_CIRCULATION;
        return STATUS_SET_ONLY;
    }
